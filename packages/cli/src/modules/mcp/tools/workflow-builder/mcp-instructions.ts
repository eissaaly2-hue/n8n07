...
Credentials: nodes added without a credential get one auto-assigned server-side, and when another node in the workflow already uses a credential of the required type, that same credential is reused. So when adding a node for a service the workflow already uses (e.g. a second Slack node next to an existing Slack trigger), leave the credential unset instead of picking one from list_credentials. Only set a credential explicitly when the workflow does not yet use that credential type — and if list_credentials returns more than one credential of that type, ask the user which one to use rather than picking one yourself.
